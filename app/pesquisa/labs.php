...
$projects = research_projects_data('laboratorio');
...
page_header($selected ? (string)$selected['title'] : 'Laboratorios de Pesquisa');
?>
<div class="container py-4">
    <?php if ($selected): ?>
        ...
        ?>
        <a class="btn btn-outline-secondary btn-sm mb-3" href="/pesquisa/labs.php">Voltar para laboratorios</a>
        <div class="card shadow-sm">
            <div id="labsCarousel" class="carousel slide" data-bs-ride="carousel">
                <div class="carousel-indicators">
                    <?php foreach ($gallery as $idx => $photo): ?>
                        <button type="button" data-bs-target="#labsCarousel" data-bs-slide-to="<?= e((string)$idx) ?>" class="<?= $idx === 0 ? 'active' : '' ?>" <?= $idx === 0 ? 'aria-current="true"' : '' ?> aria-label="Foto <?= e((string)($idx + 1)) ?>"></button>
                    <?php endforeach; ?>
                </div>
                ...
                <?php if (count($gallery) > 1): ?>
                    <button class="carousel-control-prev" type="button" data-bs-target="#labsCarousel" data-bs-slide="prev"><span class="carousel-control-prev-icon" aria-hidden="true"></span><span class="visually-hidden">Anterior</span></button>
                    <button class="carousel-control-next" type="button" data-bs-target="#labsCarousel" data-bs-slide="next"><span class="carousel-control-next-icon" aria-hidden="true"></span><span class="visually-hidden">Proxima</span></button>
                <?php endif; ?>
            </div>
            <div class="card-body">
                ...
                <?php if (!empty($selected['site_url'])): ?><a class="btn btn-primary btn-sm" href="<?= e((string)$selected['site_url']) ?>" target="_blank" rel="noopener">Site do laboratorio</a><?php endif; ?>
            </div>
        </div>
    <?php else: ?>
        <h1 class="section-title h3 mb-2">Laboratorios de Pesquisa</h1>
        <p class="text-muted mb-4">Conheca os laboratorios e linhas de trabalho desenvolvidas no DECOM.</p>
        <div class="row g-4">
            <?php foreach ($projects as $project): ?>
                <div class="col-md-6 col-xl-4">
                    <div class="card news-card card-clickable h-100 position-relative">
                        ...
                        <div class="card-body d-flex flex-column">
                            <a class="stretched-link" href="/pesquisa/labs.php?slug=<?= e((string)$project['slug']) ?>"><span class="visually-hidden">Abrir laboratorio <?= e((string)$project['title']) ?></span></a>
                            <span class="badge text-bg-primary mb-2">Laboratorio</span>
                            ...
                            <div class="mt-auto d-flex flex-wrap gap-2 position-relative" style="z-index:2;">
                                <a class="btn btn-outline-primary btn-sm" href="/pesquisa/labs.php?slug=<?= e((string)$project['slug']) ?>">Ver detalhes</a>
        ...
        <?php if (empty($projects)): ?><div class="alert alert-warning mt-4 mb-0">Nenhum laboratorio cadastrado.</div><?php endif; ?>
    <?php endif; ?>
</div>
<?php page_footer(); ?>
